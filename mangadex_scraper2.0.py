...
class MangaDexScraper:
    ...
    def download_cover(self, manga_id: str, cover_filename: str, output_dir: str = "covers") -> Optional[str]:
        """
        Download cover image for a manga
        
        Args:
            manga_id: Manga ID
            cover_filename: Cover filename from API
        ...
        # MangaDex cover URL format: https://uploads.mangadex.org/covers/{manga_id}/{filename}
        cover_url = f"https://uploads.mangadex.org/covers/{manga_id}/{cover_filename}"
        
        # Save with manga_id as filename to avoid conflicts
        file_extension = os.path.splitext(cover_filename)[1]
        local_filename = f"{manga_id}{file_extension}"
    ...
    def scrape_manga_data(self, total_manga: int = 10000, 
                         ...
                         covers_dir: str = "covers") -> List[Dict]:
        """
        ...
        Args:
            ...
            download_covers: Whether to download cover images
            covers_dir: Directory to save cover images
        """
        all_manga = []
        offset = 0
        limit = 100
        ...
        if download_covers:
            print(f"Cover images will be saved to '{covers_dir}/' directory")
        ...
        while len(all_manga) < total_manga:
            ...
            for manga in manga_list:
                ...
                if download_covers and parsed_manga.get("cover_filename"):
                    cover_path = self.download_cover(
                        parsed_manga["id"],
                        parsed_manga["cover_filename"],
                        covers_dir
                    )
                    if cover_path:
                        parsed_manga["cover_path"] = cover_path
                        covers_downloaded += 1
                ...
                if len(all_manga) % 100 == 0:
                    ...
                    cover_msg = f", {covers_downloaded} covers" if download_covers else ""
                    print(f"  ✓ {len(all_manga)} manga scraped{cover_msg} ({rate:.0f}/min)")
        ...
        if download_covers:
            print(f"✓ Downloaded {covers_downloaded} cover images to '{covers_dir}/'")
        return all_manga
    ...
    def save_to_csv(self, data: List[Dict], filename: str = "mangadex_data.csv"):
        ...
        for manga in data:
            ...
            if "cover_path" in manga:
                flat_manga["cover_path"] = manga["cover_path"]
            csv_data.append(flat_manga)
        
        keys = ["id", "title", "description", "tags", "demographic", 
                "rating", "follows", "status", "content_rating", "year"]
        
        # Add cover_path to keys if any manga has it
        if any("cover_path" in m for m in csv_data):
...
if __name__ == "__main__":
    ...
    manga_data = scraper.scrape_manga_data(
        ...
        download_covers=True,     # Set to True to download cover images
        covers_dir="covers"       # Directory to save covers
    )
